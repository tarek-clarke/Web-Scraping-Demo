...
print("Generating Master Consolidated Benchmark JSON with Named Objects and Shared QPU Latency Note...")

# Raw API data with explicit names
raw_api_data = {
    "1. OpenF1 Telemetry": {
...
# Transform per-API into named objects dictionary
...
# Programmatically calculate global_summary
...
for mk in model_keys:
    acc_list = []
    lat_list = []
    for api_name, api_models in api_specific_breakdown.items():
        if mk in api_models:
            acc_list.append(float(api_models[mk]["accuracy"].replace("%", "")))
            lat_list.append(float(api_models[mk]["latency_ms"]))
...
print(f"SUCCESS: Exported clean, named-object master JSON to {output_path}!")
