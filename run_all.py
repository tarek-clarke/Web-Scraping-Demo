...
def run_evaluation_pipeline():
 ...
 if all_res:
  ...
  for cs in st:
   print('| '+cs+' | '+'{:.2f}'.format((sum(r.get('detection_rate',0) for r in all_res if r.get('chaos_strategy')==cs)//max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))/10.0)+'% | '+'{:.2f}'.format((sum(r.get('recovery_score',0) for r in all_res if r.get('chaos_strategy')==cs)//max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))/10.0)+'% | '+'{:.3f}'.format(sum(r.get('resilience_P',0) for r in all_res if r.get('chaos_strategy')==cs)/max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))+' | '+'{:.3f}'.format(sum(r.get('resilience_P2',0) for r in all_res if r.get('chaos_strategy')==cs)/max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))+' |')
