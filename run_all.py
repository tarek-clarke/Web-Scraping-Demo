...
def _load_historical_profile_estimates(r,p): e={'short':{'average_sec':None},'long':{'average_sec':None}}; [exec('s=os.path.join(r,x,"summary.json");' 'if os.path.exists(s):' 'with open(s) as f: d=json.load(f);' 'a=d.get("average_runtime_sec");' 'if a is not None: e[x]["average_sec"]=float(a)') for x in p]; return e
