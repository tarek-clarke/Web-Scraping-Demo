...
def run_evaluation_pipeline():
 ...
 if all_res:
  hw_map={'NVIDIA CUDA':'CUDA','AMD ROCm':'ROCm','Intel GPU':'IntelGPU','Apple Silicon MPS':'MPS','CPU fallback':'CPU'};aud_plat=[hw_map.get(r.get('actual_device',''),'CPU') for r in all_res];aud_rows=[{**r,'hardware_platform':hp,'hardware_model':r.get('hardware_model',''),'cloud_environment':r.get('cloud_platform',''),'timestamp_ns':int(time.time_ns()),'strategy_used':r.get('chaos_strategy',''),'confidence_score':r.get('averages',{}).get('gemma_confidence',0)} for r,hp in zip(all_res,aud_plat)];json.dump(aud_rows,open(pdir+'/drift_reconciliation_audit.json','w'));gal_aud=list(aud_rows);[gal_aud.extend(json.load(open('results/'+ee+'/drift_reconciliation_audit.json'))) for ee in os.listdir('results/') if os.path.isdir('results/'+ee) and ee!=h and os.path.exists('results/'+ee+'/drift_reconciliation_audit.json')];json.dump(gal_aud,open('results/global_drift_reconciliation_audit.json','w'));n=len(all_res);ll=sum(r.get('averages',{}).get('levenshtein_latency',0) for r in all_res)//n;rl=sum(r.get('averages',{}).get('regex_latency',0) for r in all_res)//n;bl=sum(r.get('averages',{}).get('bert_latency',0) for r in all_res)//n;gl=sum(r.get('averages',{}).get('gemma_latency',0) for r in all_res)//n;print('\n'+'='*80);print('                     EVALUATION PIPELINE RESULTS SUMMARY');print('='*80);print('PERFORMANCE VALIDATION: BASELINE RECONCILIATION LATENCY');print('Hardware: '+d["device"].upper()+' ('+d["model"]+') | Cloud: LOCAL');print('| Algorithm | p50 Latency (ms) | p95 Latency (ms) |');print('| Levenshtein | '+'{:.3f}'.format(ll/1000.0)+' ms | '+'{:.3f}'.format(ll/1000.0)+' ms |');print('| Regex | '+'{:.3f}'.format(rl/1000.0)+' ms | '+'{:.3f}'.format(rl/1000.0)+' ms |');print('| Bert | '+'{:.3f}'.format(bl/1000.0)+' ms | '+'{:.3f}'.format(bl/1000.0)+' ms |');print('| Gemma | '+'{:.3f}'.format(gl/1000.0)+' ms | '+'{:.3f}'.format(gl/1000.0)+' ms |');print('='*80);c1=[r for r in all_res if r.get('concurrency')==1];c2=[r for r in all_res if r.get('concurrency')==2];b_t=sum(r.get('throughput_pps',0) for r in c1)//len(c1) if c1 else 0;b_e=sum(r.get('elapsed_seconds',0) for r in c1)//len(c1) if c1 else 0;p_t=sum(r.get('throughput_pps',0) for r in c2)//len(c2) if c2 else 0;p_e=sum(r.get('elapsed_seconds',0) for r in c2)//len(c2) if c2 else 0;o=((p_e-b_e)*100//max(1,b_e)) if b_e and c2 else 0;print('PERFORMANCE VALIDATION: CONCURRENCY & SCALING SCENARIOS');print('Hardware: '+d["device"].upper()+' ('+d["model"]+') | Cloud: LOCAL');print('| Mode | Average Throughput (packets/sec) | Latency Overhead Delta (%) |');print('| 1-Stream Concurrency (Base) | '+'{:.1f}'.format(b_t)+' packets/sec | 0.00% (Baseline) |');if c2:print('| 2-Stream Concurrency (Parallel) | '+'{:.1f}'.format(p_t)+' packets/sec (per stream) | '+'{:.2f}'.format(o/100.0)+'% overhead |');else:print('| 2-Stream Concurrency (Parallel) | N/A (Not Evaluated) | N/A |');print('='*80);from tests.performance.frequency_stability import print_frequency_stability;print_frequency_stability();print('='*80);print('PERFORMANCE VALIDATION: LLM CHAOS VS OTHER STRATEGIES COMPARISON');print('Hardware: '+d["device"].upper()+' ('+d["model"]+') | Cloud: LOCAL');print('| Chaos Strategy | Detection Rate (%) | Recovery Score (%) | Resilience P | Resilience P2 |');st=set(r.get('chaos_strategy','') for r in all_res);list(print('| '+cs+' | '+'{:.2f}'.format((sum(r.get('detection_rate',0) for r in all_res if r.get('chaos_strategy')==cs)//max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))/100.0)+'% | '+'{:.2f}'.format((sum(r.get('recovery_score',0) for r in all_res if r.get('chaos_strategy')==cs)//max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))/100.0)+'% | '+'{:.3f}'.format(sum(r.get('resilience_P',0) for r in all_res if r.get('chaos_strategy')==cs)/max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))+' | '+'{:.3f}'.format(sum(r.get('resilience_P2',0) for r in all_res if r.get('chaos_strategy')==cs)/max(1,sum(1 for r in all_res if r.get('chaos_strategy')==cs)))+' |') for cs in st)
