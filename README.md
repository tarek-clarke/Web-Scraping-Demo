...
`single` requests one process with two LUMI GCDs: one physical MI250X card and its full 128 GB memory. `full-node` requests four processes, each with two GCDs: four physical MI250X cards and 512 GB aggregate memory. The oracle launches one isolated worker per physical card, assigns each worker a deterministic disjoint record shard, and merges only after all shards complete. Hugging Face `device_map=auto` can therefore place the Qwen model across both GCDs belonging to that card. The subsequent VQC optimizer starts remain intentionally one-GCD jobs because the 13-qubit statevector does not benefit from multi-GCD distribution.
...
Each command builds a profile-specific oracle, launches 10 independent one-GCD VQC optimizer starts, and selects the best model only after every start succeeds. The oracle profile is 1x128 GB or 4x128 GB; each 13-qubit optimizer remains one GCD because distributing such a small statevector would add communication overhead without improving the statistical experiment. Ten starts address optimizer initialization sensitivity; they are not presented as ten independent datasets. Final claims should use the untouched held-out test split, per-API breakdowns, class balance, abstention rate, and confidence intervals across explicitly repeated experimental runs.
